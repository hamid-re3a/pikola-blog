...
@section('content')
    <article class="single-blog">
        <div class="post-thumb">
            <img src="{{ $post->featured_image ? asset($post->featured_image) : asset('assets/images/default.jpg') }}" alt="{{ $post->title }}">
        </div>
        <div class="post-content">
            <div class="entry-header text-center text-uppercase">
                <a href="#" class="post-cat">{{ $post->category->name ?? 'Uncategorized' }}</a>
                <h2>{{ $post->title }}</h2>
            </div>
